...
namespace ds::adt {
	...
	T HashTable<K, T>::remove(K key) {
		...
		if (primaryRegion_->access(index)) {
			synonyms = primaryRegion_->access(index)->data_;
		}
	...
	/*	BVSNodeType* newNode = nullptr;
		if (this->isEmpty()) {
			...
			BVSNodeType* nodeInRelation = nullptr;
			if (tryFindNodeWithKey(key,nodeInRelation)) {
				this->error("Table already contains element associated with given key!");
			}
			newNode = key > nodeInRelation->data_.key_ ? &this->getHierarchy()->insertRightSon(*nodeInRelation)
				: &this->getHierarchy()->insertLeftSon(*nodeInRelation);
		}
	...
	bool GeneralBinarySearchTree<K, T, BlockType>::tryFindNodeWithKey(K key, BVSNodeType*& node) const
	{
		...
		//while (node->data_.key_ != key && (!this->getHierarchy()->hasLeftSon(*node) && !this->getHierarchy()->hasRightSon(*node))) {
	...
	/*	BVSNodeType* leftSon = node->left_;
		...
		BVSNodeType* grandParent = getHierarchy()->accessParent(*parent);
