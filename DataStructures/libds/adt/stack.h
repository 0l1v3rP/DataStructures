...
namespace ds::adt {
    ...
    T ImplicitStack<T>::pop()
    {
        ...
        T retValue = this->getSequence()->accessLast()->data_;
    ...
    template<typename T>
    ExplicitStack<T>::ExplicitStack(const ExplicitStack& other) : ADS<T>(new amt::SinglyLS<T>(), other) {}
    ...
    T ExplicitStack<T>::pop()
    {
        ...
        T retValue = this->getSequence()->accessFirst()->data_;
