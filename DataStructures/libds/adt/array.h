...
namespace ds::adt {
	...
	ADT& Array<T>::assign(const ADT& other)
	{
		if (this != &other) {
			...
			if (this->base_ == otherArray.base_ && this->size() == otherArray.size()) {
				...
				this->error("Array<T>::assign: Arrays have different bases or sizes.");
			}
		}
		return *this;
	}
	...
	T Array<T>::access(long long index) const
	{
		if (!this->validateIndex(index)) {
			this->error("Array<T>::access: Invalid index.");
		}
	...
	{
		if (!this->validateIndex(index)) {
			this->error("Array<T>::set: Invalid index.");
		}
		size_t seqIndex = this->mapIndex(index);
	...
	T CompactMatrix<T>::access(long long index1, long long index2) const
	{
		if (!this->validateIndices(index1, index2)) {
			this->error("CompactMatrix<T>::access: Invalid index.");
		}
		size_t seqIndex = this->mapIndices(index1, index2);
		return this->getSequence()->access(seqIndex)->data_;
	}
	...
	{
		if (!this->validateIndices(index1, index2)) {
			this->error("CompactMatrix<T>::set: Invalid index.");
		}
		size_t seqIndex = this->mapIndices(index1, index2);
		this->getSequence()->access(seqIndex)->data_ = element;
	}
	...
	{
		return
			index1 >= this->dimension1_.getBase() && index1 < this->dimension1_.getBase() + static_cast<long long>(this->dimension1_.getSize()) &&
			index2 >= this->dimension2_.getBase() && index2 < this->dimension2_.getBase() + static_cast<long long>(this->dimension2_.getSize());
	}

	template<typename T>
	size_t CompactMatrix<T>::mapIndices(long long index1, long long index2) const
	{
		return (index1 - this->dimension1_.getBase()) * this->dimension2_.getSize() + (index2 - this->dimension2_.getBase());
	}
